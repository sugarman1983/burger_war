...
class EnemyDetector:
    ...
    def findEnemy(self, scan, pose_x, pose_y, th):
        ...
        enemy_scan = [1 if self.is_point_emnemy(x,i) else 0 for i,x in  enumerate(near_scan)]
            ...
            #           else:
            #               enemy_scan.appen(0)
        is_near_enemy = sum(enemy_scan) > 10  # 
...
class ChiliCheeseBurger():
    ...
    def cruise(self):
        #twist = Twist()
        #self.area = 0
        while not 0 <= self.cruise_idx < 16:
            if self.cruise_idx > 0:
                self.cruise_idx -= 16
            elif self.cruise_idx < 0:
                self.cruise_idx += 16
        
        if self.cruise_idx == 0:
            self.setGoal(-0.9,0.4,3.1415/4)
            #self.client.cancel_goal
            self.cruise_idx += self.cruise_dir
            
        elif self.cruise_idx == 1:
            self.setGoal(-0.9,-0.4,-3.1415/4)
            #self.client.cancel_goal
            self.cruise_idx += self.cruise_dir

        elif self.cruise_idx == 2:
            self.setGoal(-0.6,0,0)
            #self.client.cancel_goal
            self.cruise_idx += self.cruise_dir

        elif self.cruise_idx == 3:
        ...
        elif self.cruise_idx == 4:
            self.setGoal(0,-0.6,0)
            #self.client.cancel_goal
            self.cruise_idx += self.cruise_dir

        elif self.cruise_idx == 5:
            self.setGoal(0,-0.6,3.1415/2)
            #self.client.cancel_goal
            self.cruise_idx += self.cruise_dir

        elif self.cruise_idx == 6:
            self.setGoal(0,-0.6,3.1415)
            #self.client.cancel_goal
            self.cruise_idx += self.cruise_dir

        elif self.cruise_idx == 7:
            if self.cruise_dir == 1:
                self.setGoal(0.3,-0.3,3.1415/4)
            else:
                self.setGoal(0.3,-0.3,-3.1515*3/4)
            #self.client.cancel_goal
            self.cruise_idx += self.cruise_dir

        elif self.cruise_idx == 8:
            self.setGoal(0.9,-0.4,-3.1415*3/4)
            #self.client.cancel_goal
            self.cruise_idx += self.cruise_dir

        elif self.cruise_idx == 9:
            self.setGoal(0.9,0.4,3.1415*3/4)
            #self.client.cancel_goal
            self.cruise_idx += self.cruise_dir

        elif self.cruise_idx == 10:
            self.setGoal(0.6,0,3.1415)
            #self.client.cancel_goal
            self.cruise_idx += self.cruise_dir

        elif self.cruise_idx == 11:
            if self.cruise_dir == 1:
                self.setGoal(0.3,0.3,3.1415*3/4)
            else:
                self.setGoal(0.3,0.3,-3.1415/4)
        ...
        elif self.cruise_idx == 12:
            self.setGoal(0,0.6,3.1415)
            #self.client.cancel_goal
            self.cruise_idx += self.cruise_dir

        elif self.cruise_idx == 13:
            self.setGoal(0,0.6,-3.1415/2)
            #self.client.cancel_goal
            self.cruise_idx += self.cruise_dir

        elif self.cruise_idx == 14:
            self.setGoal(0,0.6,0)
            #self.client.cancel_goal
            self.cruise_idx += self.cruise_dir

        elif self.cruise_idx == 15:
            if self.cruise_dir == 1:
                self.setGoal(-0.3,0.3,-3.1415*3/4)
            else:
                self.setGoal(-0.3,0.3,3.1415/4)
    ...
    def strategy(self):
        ...
        while not rospy.is_shutdown():
            ...
            '''
            self.EnemyScanAverage.append(self.is_near_enemy)
            print(self.EnemyScanAverage)
            if len(self.EnemyScanAverage) > 3:
                del self.EnemyScanAverage[0]
            '''    

            if self.is_near_enemy:   # if less than 5 points, maybe noise

                print("Detect Enemy!")
                print("enemy position x:{}, y:{}".format(self.pose_x, self.point_y))
                if self.enemy_dist < 0.36:
                    ...
                    if (self.pose_y <= -self.pose_x and self.pose_y >= self.pose_x):
                        if (self.point_y <= self.point_x and self.point_y <= -self.point_x):
                            self.cruise_idx = 14
                            self.cruise_dir = -self.cruise_dir
                            print("near_enemy_cruise_idx:{}, cruise_dir:{}".format(self.cruise_idx, self.cruise_dir))
                            self.cruise()
                        elif (self.point_y >= self.point_x and self.point_y > -self.point_x):
                            self.cruise_idx = 4
                            self.cruise_dir = -self.cruise_dir
                            print("near_enemy_cruise_idx:{}, cruise_dir:{}".format(self.cruise_idx, self.cruise_dir))
                            self.cruise()
                ...
                print("cruise_idx:{}, cruise_dir:{}".format(self.cruise_idx, self.cruise_dir))
                self.cruise()  
